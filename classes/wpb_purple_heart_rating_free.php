...
class WPB_Purple_Heart_Rating_Free extends WPB_Plugin {
	 ...
	 * @since 1.0
	 */
	protected $_plugin_textdomain = 'wpbph';
	 ...
	 * @since 1.0
	 */
	protected $_plugin_version = '1.0.3';
	...
	public function add_rating_content( $content ) {
		...
		if( ! is_array( $post_types_selected ) ) $post_types_selected = array();

		// check whether to display the rating on this post type
		if( ! in_array( $post->post_type, $post_types_selected ) ) return $content;
	...
	public function options_standards( $option ) {
		...
		if( isset( $standards[$option] ) ) return $standards[$option];
	...
	public function enqueue_scripts_frontend() {
		...
		// check whether to display the rating on this post type
		if( ! in_array( $post->post_type, $post_types_selected ) ) return;
	...
	public function ajax_rate() {
		...
		if( 'wpbph-button-ok' == $_REQUEST['button_pressed'] ) $ratings['ok'] += 1;

		// if icon was pressed
		if( false != strstr( $_REQUEST['button_pressed'], 'wpbph-heart-big' ) ) $ratings['ok'] += 1;

		// if negative button was pressed
		if( 'wpbph-button-bad' == $_REQUEST['button_pressed'] ) $ratings['bad'] += 1;

		// if the small negative button was pressed
		if( false != strstr( $_REQUEST['button_pressed'], 'wpbph-heart-small' ) ) $ratings['bad'] += 1;
	...
	public static function get_rating( $post_id, $return = 'both' ) {
		...
		if( is_serialized( $ratings ) ) $ratings = unserialize( $ratings );
		...
		if( 'ok' == $return ) return $ratings['ok'];
		if( 'bad' == $return ) return $ratings['bad'];
	...
	public function get_rating_percent( $post_id ) {
		...
		if( 0 == $ratings['ok'] && 0 == $ratings['bad'] ) return array( 'ok' => 50, 'bad' => 50 );
	...
	private function set_cookie( $post_id, $post_type = 'post' ) {
		...
		if( isset( $_COOKIE['wpbph_rating_cook'] ) ) {
			...
			if( is_serialized( $cookie ) ) $cookie = @unserialize( $cookie );

			if( false == $cookie ) $cookie = array();
	...
	private function is_post_in_cookie( $post_id, $post_type = 'post' ) {
		...
		if( ! isset( $_COOKIE['wpbph_rating_cook'] ) ) return false;

		$cookie = urldecode( $_COOKIE['wpbph_rating_cook'] );

		// return false if cookie is not serialized
		if( ! is_serialized( $cookie ) ) return false;

		// get the cookie
		$cookie = @unserialize( $cookie );

		if( false == $cookie ) return false;

		// return false if this is not an array
		if( ! isset( $cookie[$post_type][$post_id] ) ) return false;
	...
	public function wp_pointer_message() {
		...
		if( ! (bool) $this->get_option( 'show_tracking_popup' ) ) return;
	...
	private function import_gd() {
		...
		if( 'import_gd' != $_GET['action'] ) return;
	...
	private function calculate_rating_percent( $oks, $bads = 0 ) {
		if( isset( $oks['bad'] ) ) $bads = $oks['bad'];
		if( isset( $oks['ok'] ) ) $oks = $oks['ok'];
