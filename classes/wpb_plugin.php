...
class WPB_Plugin implements WPB_Plugin_Interface {
	...
	public function site_transient_update_plugins( $trans ) {
		...
		if( ! $remote_plugins ) return $trans;
		...
		foreach( get_plugins() as $plugin_slug => $plugin ) {
			...
			if( ! isset( $remote_plugins[$plugin_slug] ) ) continue;
			...
			if( version_compare( $plugin['Version'], $remote_plugins[$plugin_slug]->version, '<' ) ) {
				...
				if( isset( $trans->response[$plugin_slug] ) ) unset( $trans->response[$plugin_slug] );
			}
		}
	...
	public function plugins_api( $api, $action, $args ) {
		...
		if( false !== $api || ! isset( $args->slug ) || $slug != $args->slug && $args->slug != $slug ) return false;
		...
		if( 'plugin_information' == $action ) {
			...
			if( ! isset( $plugins[$extended_slug] ) ) return false;
	...
	public function get_client_upgrade_data() {
		...
		if( isset( $wpb_has_plugin_remote_sent[$this->get_plugin_name_sanitized()] ) ) return $wpb_has_plugin_remote_sent[$this->get_plugin_name_sanitized()];

		// if the plugins are already set, return it!
		$plugins = get_site_transient( 'wpb_plugin_update_' . $this->get_plugin_name_sanitized() );

		global $pagenow;

		if( 'update-core.php' == $pagenow ) {
			//update check no matter what $plugins returns
		}

		// on update-core.php pages check for a new update everytime the user visits it
		if( 'update-core.php' != $pagenow ) {
			// stop here if the transient has been set
			if( false !== $plugins ) return $plugins;
		}
		...
		if( ! is_wp_error( $data ) && 200 == wp_remote_retrieve_response_code( $data ) ) {
			if( $body = json_decode( wp_remote_retrieve_body( $data ), true ) ) {
				if( is_array( $body ) && isset( $body['plugins'] ) && is_serialized( $body['plugins'] ) ) {
					...
					set_site_transient( 'wpb_plugin_update_' . $this->get_plugin_name_sanitized(), $remote_plugins, 60 * 60 * 24 );
	...
	public function plugins_url( $path, $plugin = null ) {
		if( is_null( $plugin ) ) $plugin = $this->_plugin_file;
		if( 'plugin' == $this->_inclusion ) return plugins_url( $path, $plugin );
		else return get_template_directory_uri() . '/' . $path;
	}
	...
	public function theme_on_activation() {
		...
		if( $pagenow == 'themes.php' && isset( $_REQUEST['activated'] ) ) {
			...
			if( method_exists( $this, 'on_activation' ) ) $this->on_activation();
	...
	public function get_textdomain() {
		if( isset( $this->_plugin_textdomain ) ) return $this->_plugin_textdomain;
		return '';
	}
	...
	public function set_plugin_path( $file = null ) {
		...
		if( is_null( $this->_plugin_path ) ) $this->_plugin_path = dirname( __FILE__ );
	}
	...
	public function track( $tasks ) {
		...
		if( ! method_exists( $this, 'is_tracking' ) ) return;
	...
	public function get_plugin_version() {
		if( isset( $this->_plugin_version ) ) return $this->_plugin_version;
		return 0;
	}
