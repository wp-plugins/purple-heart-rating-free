jQuery( document ).ready( function() {
	...
	jQuery( '.wpbph-ajax-loader' ).ajaxStart( function() {
		ajax_loader_image_obj.show().css( 'display', 'inline-block' );
		is_ajax_request = true;
	} );

	jQuery( '.wpbph-ajax-loader' ).ajaxStop( function() {
	...
	function wpbph_pretty() {
		jQuery( '.wpbph-frontend' ).each( function() {
			...
			if( width < 600 ) {
				jQuery( this ).find( '.wpbph-info, .wpbph-rating' ).width( '100%' );
			} else {
				jQuery( this ).find( '.wpbph-info, .wpbph-rating' ).width( '290px' );
			}
		} );
	}
	...
	jQuery( '.wpbph-button-ok, .wpbph-button-bad, .wpbph-table-big-heart i, .wpbph-heart-small' ).click( function( event ) {
		...
		if( is_ajax_request || rated_on_this_page ) return false;
		...
		if( thisObj.hasClass( 'wpbph-heart-small' ) ) post_id = thisObj.parent().parent().parent().data( 'post_id' );

		ajax_loader_image_obj = thisObj.closest( '.wpbph-rating' ).find( '.wpbph-ajax-loader' );

		jQuery.post( WPBAjaxRating.ajaxurl, { 'action':'wpbph_ajax_rate', 'post_id':post_id, 'button_pressed':button_pressed }, function( response ) {
			if( 1 == response.error ) {
	...
	function wpbph_run_up( e, from, to, after_func ) {
		...
		if( 0 == to ) {
			if( is_function( after_func ) ) after_func();
			return;
		}
	...
	function refresh_ratings() {
		jQuery.post( WPBAjaxRating.ajaxurl, { 'action':'wpbph_ajax_refresh_post_ratings', 'post_id':WPBAjaxRating.current_post_id }, function( response ) {
			if( 1 == response.error ) {
